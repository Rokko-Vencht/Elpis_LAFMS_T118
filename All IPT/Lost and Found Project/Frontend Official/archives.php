...
<head>
    ...
    <title>FoundIt: Reunited Items</title>
    ...
    <style>
        ...
        .lf-modal-container.show, .lf-modal-container-f.show, .lf-modal-container-view-response.show {
            display: flex;
        }
...
<body>
    <div id="pge-cont">
        <div id="topbar">
            ...
            <div class="srch-cont">
                <form action="archives.php" method="GET" class="srch-grp">
        ...
        <div id="bttm-cont">
            ...
            <div id="main-cont">
                ...
                <div class="main-cont-header">
                    <h1>Reunited Items</h1>
                    <p>See the archived / past transactions of items that were resolved.</p>
                </div>
                
                <form action="generate_report.php" method="POST" style="align-self: flex-end;" target="_blank">
                    <button type="submit" class="generate-reports-btn" id="generate-reports-btn">Generate Reports</button>
                </form>
                
                <?php if (!empty($searchTerm)): ?>
                <div class="search-results-header">
                    <p>Search results for: <strong><?php echo htmlspecialchars($_GET['search']); ?></strong></p>
                    <a href="archives.php" class="clear-search-btn">Clear Search</a>
                </div>
                <?php endif; ?>
                ...
                <div class="lf-items-container">
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while($row = $result->fetch_assoc()) {
                            ...
                            <!-- INSERT HTML FORMATTING HERE WHILE ALSO INJECTING ALL OF THE INPUTS WE FETCHED FROM LAFCMS_SAMPLE.DB -->
    ...
    <script>
        ...
        // Simplified script for archives page
        document.addEventListener('DOMContentLoaded', function() {
            // View details buttons
            document.querySelectorAll('.lf-view-details-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    if (imgElement) imgElement.src = itemData.itemImage;
                    
                    // Update text fields (always set these)
                    const setIfExists = (selector, value) => {
                        const el = modalContainer.querySelector(selector);
                        if (el) el.textContent = value;
                    };
                    setIfExists('.lf-item-name', itemData.itemName);
                    setIfExists('.lf-report-status', itemData.reportStatus);
                    setIfExists('.lf-category-name', itemData.categoryName);
                    setIfExists('.lf-date-filed', itemData.dateFiled);
                    setIfExists('.lf-pub-id', itemData.pubName);
                    setIfExists('.lf-desc-content', itemData.itemDetails || 'No description provided.');
                    
                    // Set fields specific to lost or found
                    if (reportStatus === 'Lost') {
                        setIfExists('.lf-found-location', itemData.foundLocation || 'N/A');
                        setIfExists('.lf-transaction-status', itemData.transactionStatus || 'N/A');
                    }
                    if (reportStatus === 'Found') {
                        setIfExists('.lf-found-location', itemData.foundLocation || 'N/A');
                        setIfExists('.lf-stored-location', itemData.storedLocation || 'N/A');
                        setIfExists('.lf-claim-status', itemData.claimStatus || 'Unclaimed');
                        setIfExists('.lf-user-respo', itemData.userRespoName || 'N/A');
                        setIfExists('.lf-transaction-status', itemData.transactionStatus || 'N/A');
                    }
            ...
            // Close buttons
            document.querySelectorAll('#lf-close, #lf-close2, #lf-close-view-response').forEach(button => {
                button.addEventListener('click', function() {
                    const modal = this.closest('.lf-modal-container, .lf-modal-container-f, .lf-modal-container-view-response');
                    if (modal) {
